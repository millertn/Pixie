...
export class ImportToolService {
    ...
    public loadState(data: string|SerializedCanvas): Promise<void> {
        return this.resetEditor().then(() => {
            return this.history.addFromJson(data).then(() => {
                this.store.dispatch(new ContentLoaded()).subscribe(() => {
     ...
     */
    public openBackgroundImage(data: string|HTMLImageElement, options: {resetEditor?: boolean} = {resetEditor: false}): Promise<Image> {
